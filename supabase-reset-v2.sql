...
-- 2. Funcion helper en public (no auth)
...
create policy "own months"       on public.months       for all using (user_id = public.clerk_user_id()) with check (user_id = public.clerk_user_id());
create policy "own habits"       on public.habits       for all using (user_id = public.clerk_user_id()) with check (user_id = public.clerk_user_id());
create policy "own days"         on public.days         for all using (user_id = public.clerk_user_id()) with check (user_id = public.clerk_user_id());
create policy "own goals"        on public.goals        for all using (user_id = public.clerk_user_id()) with check (user_id = public.clerk_user_id());
create policy "own tweaks"       on public.tweaks       for all using (user_id = public.clerk_user_id()) with check (user_id = public.clerk_user_id());
create policy "own ui_state"     on public.ui_state     for all using (user_id = public.clerk_user_id()) with check (user_id = public.clerk_user_id());
create policy "own users"        on public.users        for all using (id = public.clerk_user_id())      with check (id = public.clerk_user_id());
create policy "own login_events" on public.login_events for all using (user_id = public.clerk_user_id()) with check (user_id = public.clerk_user_id());
