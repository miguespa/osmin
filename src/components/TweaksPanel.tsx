...
export function useTweaks(defaults: Tweaks): [Tweaks, (keyOrEdits: keyof Tweaks | Partial<Tweaks>, val?: Tweaks[keyof Tweaks]) => void] {
  const [values, setValues] = useState<Tweaks>(defaults)
  const setTweak = useCallback((keyOrEdits: keyof Tweaks | Partial<Tweaks>, val?: Tweaks[keyof Tweaks]) => {
    const edits: Partial<Tweaks> = typeof keyOrEdits === 'object' && keyOrEdits !== null
      ? keyOrEdits : { [keyOrEdits]: val } as Partial<Tweaks>
...
export function TweakRow({ label, value, children, inline = false }: { label: string; value?: string | number; children?: ReactNode; inline?: boolean }) {
  return (
...
export function TweakColor({ label, value, options, onChange }: { label: string; value: string | string[]; options?: ColorOption[]; onChange: (v: string | string[]) => void }) {
